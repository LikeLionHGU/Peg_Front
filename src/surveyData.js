const multiplechoicetest = [
  {
    questionID: "field1",
    title: "상대와의 카톡방이 개설되었다, 그때 당신의 속마음은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "오...어떤 사람일까.. 내가 실수하면 어떡하지?",
      },
      {
        answerID: 2,
        answerText: "앗싸! 친구한테 소개남(녀)의 정보를 물어봐야겠어.",
      },
      {
        answerID: 3,
        answerText: "전연애와 똑같으면 어떡하지",
      },
      {
        answerID: 4,
        answerText: "언제 만나자고 하지? 취미부터 공유해봐야겠다.",
      },
    ],
  },
  {
    questionID: "field2",
    title:
      "나를 좋아하는 남사친(여)과 데이트를 하는 중에 많이 사랑했었던 전남친과 마주쳤다. 그날 밤 전남친(여)에게서 “둘이 무슨 사이야?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "전남친은 끝난 사이지, 왜 연락오는거야?",
      },
      {
        answerID: 2,
        answerText: "아직 못 잊은건가...그때 우리 참 예뻤었는데",
      },
      {
        answerID: 3,
        answerText: "똥차 가고 벤츠 온다.",
      },
      {
        answerID: 4,
        answerText: "얘는 이제와서 왜 이러는 걸까...다시 잘해볼려고?",
      },
    ],
  },
  {
    questionID: "field3",
    title:
      "시험끝나고 친구들끼리 영일대에 놀러 나갔다. 바다를 보던 중 한 무리의 이성이 번호를 따려고 한다. 그때의 당신은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "사람은 많이 다양하게 만나봐야되는겨. 번호주자",
      },
      {
        answerID: 2,
        answerText: "내가 쉬워보이나? 헌팅은 조심하라고 다들 그러던데...",
      },
      {
        answerID: 3,
        answerText: "내가 잘생겨(예뻐) 보이는군. 역시",
      },
      {
        answerID: 4,
        answerText: "이러다 큰일 일어나는 건 아니겠지?",
      },
    ],
  },
  {
    questionID: "field4",
    title: "애인과 데이트하는 중에 친한 이성친구가 전화가 왔다. 당신의 반응은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "전화가 왔네? 받아야지",
      },
      {
        answerID: 2,
        answerText: "데이트중엔 애인한테 집중해야겠다.",
      },
      {
        answerID: 3,
        answerText: "급한 일이 있나..?(애인의 눈치를 본다)",
      },
      {
        answerID: 4,
        answerText: "아 눈치없네",
      },
    ],
  },
  {
    questionID: "field5",
    title:
      "나 좋다고한 소개남(녀)가 하루만에 인생네컷을 찍자고 한다. 당신의 반응은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "사진 정도야 뭐 친구랑도 찍으니까",
      },
      {
        answerID: 2,
        answerText: "너무 빠르지 않나...",
      },
      {
        answerID: 3,
        answerText: "에이, 너무 쉬우면 썸이냐 튕기자",
      },
      {
        answerID: 4,
        answerText: "어떤 포즈로 해야 적절할까?",
      },
    ],
  },
  {
    questionID: "field6",
    title: "애인이 이성친구를 두고 칭찬을 많이 한다. 그때 당신의 반응은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "걔는 걔고 나는 나지",
      },
      {
        answerID: 2,
        answerText: "애인 눈에도 00이 괜찮아보이나...신경쓰인다.",
      },
      {
        answerID: 3,
        answerText: "죽고싶나...",
      },
      {
        answerID: 4,
        answerText: "친구의 어떤 점이 내가 본받을만 할까?",
      },
    ],
  },
  {
    questionID: "field7",
    title:
      "애인이 내가 좋아하지 않는 친구랑 팀가족이 되어, 가족밥고를 한다고 한다. 당신은 어떤 반응을 보이나요?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "마음에 안 들지만 관여안한다.",
      },
      {
        answerID: 2,
        answerText: "본인이랑은 사이좋으니까...이해하자.",
      },
      {
        answerID: 3,
        answerText: "내가 싫다는데 왜 말을 안 듣지?",
      },
      {
        answerID: 4,
        answerText: "한번 더 말해본다.",
      },
    ],
  },
  {
    questionID: "field8",
    title:
      "애인이 내 입냄새가 너무 심해서 힘들다고 토로한다. 당신의 다음 반응은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "사람은 입냄새 다 나, 신경쓰지 말자",
      },
      {
        answerID: 2,
        answerText: "너무 민망한데...",
      },
      {
        answerID: 3,
        answerText: "어쩌라고...",
      },
      {
        answerID: 4,
        answerText: "엇...나 칫솔질 자주 하는데...치과를 가봐야되나",
      },
    ],
  },
  {
    questionID: "field9",
    title:
      "애인이 내가 좋아하지 않는 친구랑 팀씨씨를 한다고 한다. 바로 나오는 당신의 반응은? ",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "신경쓰지 않는다.",
      },
      {
        answerID: 2,
        answerText: "혼자 속으로 삭힌다.",
      },
      {
        answerID: 3,
        answerText: "화 그 자체로 낸다.",
      },
      {
        answerID: 4,
        answerText: "침착하게 논리적으로 권유한다.",
      },
    ],
  },
  {
    questionID: "field10",
    title:
      "썸타는 사람이 고백과 함께 자신의 얼마남지 않은 군대 영장을 보여주었다. 당신의 선택과 반응은?",
    optionType: "B",
    option: [
      {
        answerID: 1,
        answerText: "고백을 거절한다.",
      },
      {
        answerID: 2,
        answerText: "어...어떡하지...친구한테 상담받아야겠다..",
      },
      {
        answerID: 3,
        answerText: "어이가 없네",
      },
      {
        answerID: 4,
        answerText:
          "18개월동안 내조를 잘해줘야겠다! 내 할 것 잘하고 있으면 되징",
      },
    ],
  },
];

const oxtest = [
  {
    questionID: "field1",
    title:
      "애인이 신경쓰여하는 이성친구랑 팀가족이 되었다. 가족밥고에 나갈 것인가요?",
    optionType: "A",
  },
  {
    questionID: "field2",
    title:
      "내 졸업식에 애인이 까먹고 준비를 안했다가 당일에 꽃다발을 준비했다. 당신은 만족하시나요?",
    optionType: "A",
  },
  {
    questionID: "field3",
    title: "커플링을 보고 친구가 껴봐도 되냐고 물었다. 당신은 허락할 것인가요?",
    optionType: "A",
  },
  {
    questionID: "field4",
    title: "애인있는데 새새와 해외여행이 가능하다.",
    optionType: "A",
  },
  {
    questionID: "field5",
    title: "사귀기 전에 손잡는 것 괜찮다.",
    optionType: "A",
  },
  {
    questionID: "field6",
    title: "전애인에게 받은 고가의 선물을 계속 사용하는 것은 괜찮다.",
    optionType: "A",
  },
  {
    questionID: "field7",
    title:
      "폭우가 왔다, 애인의 우산을 애인과 이성친구가 같이 썼는 것이 상관이 없다.",
    optionType: "A",
  },
  {
    questionID: "field8",
    title: "새내기때 동새끼리 썸타는 것, 괜찮다.",
    optionType: "A",
  },
  {
    questionID: "field9",
    title:
      "내 썸타는 사람이 방돌이(방순이)의 애인이었다. 관계를 지속해 나갈 것이다.",
    optionType: "A",
  },
  {
    questionID: "field10",
    title:
      "팀에서는 화이트데이 선물을 같이 준비했지만 나한테는 안 줘도 상관없다.",
    optionType: "A",
  },
];

export const surveyData = { multiplechoicetest, oxtest };

export const activities = [
  "요즘 새로 생긴 카페 가보고 싶은데, 같이 갈 사람?",
  "주말에 등산 갈 사람 있어?",
  "이번 주말에 미술관 전시 보러 갈래?",
  "요즘 날씨 좋은데, 한강에서 자전거 탈 사람?",
  "새로운 맛집 발견했는데, 같이 가볼래?",
  "다음 주에 축구 경기 보러 갈 사람?",
  "요즘 핫한 카페투어 같이 할 사람?",
  "주말에 야구장 갈 사람 구해요.",
  "공원에서 피크닉 할 사람 있나요?",
  "요가 클래스 같이 다닐 사람 구해요.",
  "주말에 캠핑 갈 사람 있어?",
  "요즘 헬스장 등록할까 하는데, 같이 다닐 사람?",
  "주말에 도서관에서 독서 모임 할래요?",
  "한강에서 롤러블레이드 탈 사람 구해요.",
  "요즘 볼링장 재밌다고 하던데, 같이 갈래?",
  "카페에서 보드게임 할 사람 있어?",
  "다음 주말에 드라이브 갈 사람?",
  "요즘 자전거 타기 좋은데, 같이 탈 사람?",
  "주말에 스파 가서 힐링할 사람?",
  "주말에 공연 보러 갈 사람?",
];
